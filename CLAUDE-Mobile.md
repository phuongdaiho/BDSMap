...
├── sw.js                   ← Service Worker (offline cache, hiện v19)
...
- **Tab Cộng đồng**: `communityLocations` trong viewport, **loại trừ địa điểm của chính user** (`ownerUid !== currentUser?.uid`)
...
### 17. Quick View "🏠 Xem bán"

Nút **"🏠 Xem bán"** trên header (giữa badge mode và nút BĐS cá nhân).
...
### 18. Export toàn bộ dữ liệu (export.html)
...
| `renderSidebar()` | Render danh sách / nhóm theo `sidebarTab`; community tab dùng `displayCommunity` (đã lọc ownerUid) |
...
| `makePopupHtml(loc, isCommunity)` | HTML popup (gallery, truncate, 2 cột, status badge, XSS escape) |
...
| `showQuickCard(idx)` | Render card BĐS tại index vào `#qv-body`, cập nhật counter và nút |
